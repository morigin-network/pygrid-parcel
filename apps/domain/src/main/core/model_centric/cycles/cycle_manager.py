...
class CycleManager(DatabaseManager):
    ...
    def submit_worker_diff(self, worker_id: str, request_key: str, diff):
        """Submit reported diff
        Args:
             worker_id: Worker's ID.
             request_key: request (token) used by this worker during this cycle.
             diff: doc id of the model param from worker ///Model params trained by this worker.
        Returns:
             cycle_id : Cycle's ID.
        Raises:
        ...
        # raise ValueError('Can commit into the worker cyecle db!!!')
    ...
    def _average_plan_diffs(self, server_config: dict, cycle):
        ...
        # ]
        # logging.info("Success to this point!!!!!!")
        diff_ids = [id.diff for id in reports_to_average]

        avg_plan_rec = process_manager.get_plan(
            fl_process_id=cycle.fl_process_id, is_avg_plan=True
        )
        iterative_plan = str(int(server_config.get("iterative_plan", False)))
        avgplan = base64.b64encode(avg_plan_rec.value).decode("ascii")+iterative_plan if avg_plan_rec and avg_plan_rec.value else "None"
